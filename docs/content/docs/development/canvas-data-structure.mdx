...
type CanvasProjectPptStyleContract = {
  ...
  modelStyle: {
    ...
    roleMasters: Record<
      "cover" | "section" | "content" | "evidence" | "comparison" | "close",
      string
    >;
...
type CanvasProjectPptSourceRef = {
  ...
  /** verbatim = 页面文案可在 excerpt 中逐字定位；derived = 基于 excerpt 归纳/压缩，数字与大写术语须落在 excerpt 内 */
...
- `deckBrief.styleContract` 是唯一可编辑的 deck 级视觉事实源。preset/candidate ID 只记录选择来源；Compiler 只消费冻结的 `modelStyle` 并编译为全局规则、六类角色母版和 `styleFingerprint`。
- `styleContract.references` 只保存本地图片 `storageKey`，不会为参考图伪造画布节点。图片清理会把这些 key 计入使用集合。
- 视觉检查同时审查 Contract、`globalRules`、PageSpec 和当前方案构图。页级字体、背景、色板或气质覆盖会 fail-closed；确定性 repair patch 必须绑定 action ID、draft revision 和 review fingerprint，应用前重算全部 operation。
- structured 的标题、核心信息和其他正文都在 `contentBlocks`；每个非占位块必须引用能支持该文字的 `sourceRef`（`relation=verbatim` 要求文案可在 excerpt 中逐字定位；`relation=derived` 要求数字与大写术语落在 excerpt 内）。`DeckBrief.sourceHash` 将整套受众、目标和叙事定位绑定到生成内容方案时的 material + requirements；页级来源则在 Builder、生成计划和 provider POST 前重新按行号切片并比对 excerpt。任一层漂移或 `contentState` 未到 `approved` 都会阻断生成。
...
- 代表性风格校样用 `styleProof` 绑定已确认候选、Contract fingerprint 和内容版本；`styleProofCandidateIds` 记录历史校样身份。生成其余页只能引用当前校样且恰好一次，“直接生成全部”排除所有历史校样。
...
材料或要求无法支持的新增事实会成为信息缺口，不会自动伪造成事实；基于材料的压缩/归纳可在 `relation=derived` 下来源绑定而不逐句阻断。用户回答和已确认假设会新增 `relation=verbatim` 的 sourceRef；省略或占位不能用于整套定位、标题和核心信息。编辑、删页、合并、排序、安全修复与单页重生成都绑定 `revision`，过期 preview/result 原子失败；只有不存在 blocking audit issue 时才能 finalize 为 `approved` PageSpec。
