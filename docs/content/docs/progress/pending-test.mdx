...
- PPT 工作台：新增 `/ppt` 向导（材料 → 流式生成大纲并可编辑标题/要点/增删页 → 风格描述与参考图 → 建图跳转画布）与画布内 PPT 面板（全部生成含首页锚定与跳过选项、单页重新生成、确认此页/取消确认、按页序打包下载图片 zip、页结构缺失时可重建单页）；需验证六步闭环、单页生成失败不影响其他页、非 PPT 画布工程行为无回归、删除其他工程或素材后 deck 参考图与页图不被误清理。测试注意：面板触发生成后画布会自动选中并打开其中一页生成配置节点的提示词面板（生成管线既有行为，与 Agent 触发生成一致，不会连环弹窗），点击画布空白处即可关闭。
