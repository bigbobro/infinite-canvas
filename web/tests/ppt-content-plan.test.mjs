...
let acknowledgePptPrincipleDeviation;
let revokePptPrincipleDeviation;
...
let derivePptPageProvenanceGapCause;
...
// SHA-30b：normalizePage 是「构造器」——纯布局意图里夹带的未批准事实/未识别修饰词残余，
// 在系统自产初稿入库时就被静默整理掉（不建 gap、不报 issue），不再逐条留到校验阶段拖出 blocking issue。
test("SHA-30b：初稿 layoutIntent 里的事实/未识别残余在构造期被静默整理，不再逐条拖出 blocking issue", () => {
    for (const layoutIntent of ["右侧突出服务器投入 999 台", "左侧腾讯云，右侧阿里云", "右侧展示999个指标", "顶部排列999个要点", "999个指标水平排列"]) {
        const raw = rawDraft();
        ...
        assert.deepEqual(draft.pageSpecs[0].layoutIntent, [], `应整体移除：${layoutIntent}`);
        assert.ok(draft.pageSpecs[0].autoTidy?.length, `应记录整理痕迹：${layoutIntent}`);
        assert.equal(
            draft.audit.issues.some((issue) => issue.field === "layoutIntent"),
            false,
        );
        assert.equal(validatePptContentDraft(draft).valid, true);
        assert.doesNotThrow(() => finalizePptContentDraft(draft));
    }
});

// 无法安全拆分出纯样式片段的混合表述（构造期原样放行，样式路由/finalize 另行处理）仍要落到既有校验，
// 保留 Compiler 绝不放行未批准事实的兜底。
test("layoutIntent 混入无法安全分离的风格描述时，残余事实仍会被识别为 invalid_content_structure 并阻断 Compiler", () => {
    const raw = rawDraft();
    raw.pages[0].blocks = raw.pages[0].blocks.filter((block) => block.key === "known");
    raw.pages[0].gaps = [];
    raw.pages[0].visualEncoding = [];
    raw.pages[0].layoutIntent = ["深蓝科技风，右侧突出服务器投入 999 台"];
    const draft = normalizePptContentDraft(raw, sourceInput());

    assert.deepEqual(draft.pageSpecs[0].layoutIntent, ["深蓝科技风，右侧突出服务器投入 999 台"]);
    ...
    const snapshot = compilePptPromptSnapshot({
        compilePolicy: "structured",
        snapshotId: "snapshot-layout-fact-style-mixed",
        compiledAt: "2026-07-22T08:02:00.000Z",
    ...
    assert.doesNotMatch(snapshot.prompts[0].finalPrompt, /999\s*(?:台|个)/);
});

test("SHA-30b：词表外的纯修饰词残余在构造期被剥除，保留可识别部分并留痕", () => {
    const raw = rawDraft();
    raw.pages[0].blocks = raw.pages[0].blocks.filter((block) => block.key === "known");
    raw.pages[0].gaps = [];
    raw.pages[0].visualEncoding = [];
    raw.pages[0].layoutIntent = ["瀑布式·居中布局"];
    const draft = normalizePptContentDraft(raw, sourceInput());

    assert.deepEqual(draft.pageSpecs[0].layoutIntent, ["居中布局"]);
    assert.ok(draft.pageSpecs[0].autoTidy?.some((note) => note.includes("居中布局")));
    assert.equal(
        draft.audit.issues.some((issue) => issue.field === "layoutIntent"),
        false,
    );
...
test("SHA-30b：封面页与标题/核心信息同文的正文块入库时被静默清掉并留痕", () => {
    const draft = normalizePptContentDraft(
        {
            ...
            pages: [
                {
                    ...
                    contentForm: "cover",
                    blocks: [{ key: "echo", kind: "body", text: "把内容规划连接到页面生成", source: { source: "material", startLine: 2, endLine: 2 } }],
                    layoutIntent: ["居中主视觉"],
                    visualEncoding: [{ contentKeys: ["echo"], intent: "emphasize", channel: "position" }],
                    gaps: [],
                },
            ],
        },
        {
            title: "PPT 工作台",
            sourceMaterial: ["PPT 工作台", "把内容规划连接到页面生成", "第一次接触产品的人", "理解产品价值", "从问题到价值"].join("\n"),
            requirements: "",
        },
    );
    const page = draft.pageSpecs[0];
    assert.deepEqual(
        page.contentBlocks.map((block) => block.kind),
        ["title", "primary_claim"],
    );
    assert.equal(page.visualEncoding.length, 0);
    assert.equal(
        draft.audit.gaps.some((gap) => gap.pageId === page.pageId),
        false,
    );
    assert.equal(
        draft.audit.issues.some((issue) => issue.pageIds.includes(page.pageId)),
        false,
    );
    assert.ok(page.autoTidy?.some((note) => note.includes("核心信息")));
});

test("SHA-30b：封面不同文的真实内容块仍保留现状，解析期不清理", () => {
    const draft = normalizePptContentDraft(
        {
            brief: { audience: "第一次接触产品的人", goal: "理解产品价值", narrative: "从问题到价值" },
            pages: [
                {
                    ...
                    primaryClaimSource: { source: "material", startLine: 2, endLine: 2 },
                    contentForm: "cover",
                    blocks: [{ key: "extra", kind: "body", text: "另一句与标题核心信息都不同的正文", source: { source: "material", startLine: 6, endLine: 6 } }],
                    layoutIntent: [],
                    visualEncoding: [],
                    gaps: [],
                },
        ...
        {
            title: "PPT 工作台",
            sourceMaterial: ["PPT 工作台", "把内容规划连接到页面生成", "第一次接触产品的人", "理解产品价值", "从问题到价值", "另一句与标题核心信息都不同的正文"].join("\n"),
            requirements: "",
        },
    );
    const page = draft.pageSpecs[0];
    assert.equal(
        page.contentBlocks.some((block) => block.text === "另一句与标题核心信息都不同的正文"),
        true,
    );
    assert.ok(draft.audit.issues.some((issue) => issue.code === "principle_question" && issue.pageIds.includes(page.pageId)));
});

test("SHA-30c：封面承载真实内容块的理念违规从 blocking 判决变为带选项的问题卡", () => {
    const material = ["PPT 工作台", "把内容规划连接到页面生成", "另一句与标题核心信息都不同的正文", "第一次接触产品的人", "理解产品价值", "从问题到价值", "正文页要点"].join("\n");
    const draft = normalizePptContentDraft(
        {
            brief: { audience: "第一次接触产品的人", goal: "理解产品价值", narrative: "从问题到价值" },
            pages: [
                {
                    ...
                    primaryClaimSource: { source: "material", startLine: 2, endLine: 2 },
                    contentForm: "cover",
                    blocks: [{ key: "extra", kind: "body", text: "另一句与标题核心信息都不同的正文", source: { source: "material", startLine: 3, endLine: 3 } }],
                    layoutIntent: [],
                    visualEncoding: [],
                    gaps: [],
                },
                {
                    title: "正文页",
                    titleSource: { source: "material", startLine: 7, endLine: 7 },
                    purpose: "展开要点",
                    primaryClaim: "正文页要点",
        ...
        { title: "PPT 工作台", sourceMaterial: material, requirements: "" },
    );
    const coverPage = draft.pageSpecs[0];
    const nextPage = draft.pageSpecs[1];
    const extraBlock = coverPage.contentBlocks.find((block) => block.text === "另一句与标题核心信息都不同的正文");
    const issue = draft.audit.issues.find((item) => item.code === "principle_question" && item.pageIds.includes(coverPage.pageId));
    assert.ok(issue);
    assert.equal(issue.severity, "blocking");
    assert.deepEqual(
        issue.actions.find((action) => action.kind === "move_block"),
        { kind: "move_block", pageId: coverPage.pageId, blockId: extraBlock.id, targetPageId: nextPage.pageId },
    );
    assert.deepEqual(
        issue.actions.find((action) => action.kind === "remove_block"),
        { kind: "remove_block", pageId: coverPage.pageId, blockId: extraBlock.id },
    );
    assert.deepEqual(
        issue.actions.find((action) => action.kind === "acknowledge_deviation"),
        { kind: "acknowledge_deviation", pageId: coverPage.pageId, principle: "cover-extra-content" },
    );
    // 不再判决为 invalid_cover：理念问题从阻断判决改为带选项的问题卡。
    assert.equal(
        draft.audit.issues.some((item) => item.code === "invalid_cover" && item.pageIds.includes(coverPage.pageId)),
        false,
    );
});

test("SHA-30c：单页 deck 的封面理念问题卡不提供移到下一页选项", () => {
    const draft = normalizePptContentDraft(
        {
            brief: { audience: "第一次接触产品的人", goal: "理解产品价值", narrative: "从问题到价值" },
            pages: [
    ...
    const page = draft.pageSpecs[0];
    const issue = draft.audit.issues.find((item) => item.code === "principle_question" && item.pageIds.includes(page.pageId));
    assert.ok(issue);
    assert.equal(
        issue.actions.some((action) => action.kind === "move_block"),
        false,
    );
    assert.ok(issue.actions.some((action) => action.kind === "remove_block"));
    assert.ok(issue.actions.some((action) => action.kind === "acknowledge_deviation"));
});

test("SHA-30c：承接封面理念偏离后审计闭嘴，编译通过且 finalPrompt 含承接指令；撤销后恢复 blocking", () => {
    const draft = normalizePptContentDraft(
        {
            brief: { audience: "第一次接触产品的人", goal: "理解产品价值", narrative: "从问题到价值" },
            pages: [
    ...
    const page = draft.pageSpecs[0];
    const acknowledged = acknowledgePptPrincipleDeviation(draft, page.pageId, "cover-extra-content", "2026-07-23T08:00:00.000Z");
    const acknowledgedPage = acknowledged.pageSpecs[0];
    assert.deepEqual(acknowledgedPage.principleDeviations, [{ principle: "cover-extra-content", acknowledgedAt: "2026-07-23T08:00:00.000Z" }]);
    assert.equal(acknowledgedPage.version, page.version + 1);
    assert.equal(
        acknowledged.audit.issues.some((issue) => issue.pageIds.includes(page.pageId)),
        false,
    );
    assert.equal(validatePptContentDraft(acknowledged).valid, true);
    assert.deepEqual(validatePptPageSpec({ ...acknowledgedPage, contentState: { status: "approved", approvedAt: "test" } }), []);

    // 编译需要已批准内容规格（finalizePptContentDraft 的产物），而非草稿态 contentState。
    const { brief, pageSpecs } = finalizePptContentDraft(acknowledged, "2026-07-23T08:00:00.000Z");
    const finalizedPage = pageSpecs[0];
    assert.deepEqual(finalizedPage.principleDeviations, acknowledgedPage.principleDeviations);
    const snapshot = compilePptPromptSnapshot({
        compilePolicy: "structured",
        snapshotId: "snapshot-deviation-acknowledged",
        compiledAt: "2026-07-23T08:00:00.000Z",
        deckBrief: deckBriefFrom(brief),
        pageSpecs,
        deckShell: derivePptDeckShellFacts(pageSpecs, "偏离承接测试"),
        targets: [targetFor(finalizedPage)],
    });
    assert.equal(
        snapshot.issues.some((issue) => issue.severity === "blocking"),
        false,
    );
    assert.match(snapshot.prompts[0].finalPrompt, /封面在保持定位语视觉主导的前提下承载少量补充内容/);

    const revoked = revokePptPrincipleDeviation(acknowledged, page.pageId, "cover-extra-content");
    assert.equal(revoked.pageSpecs[0].principleDeviations, undefined);
    assert.ok(revoked.audit.issues.some((issue) => issue.code === "principle_question" && issue.pageIds.includes(page.pageId) && issue.severity === "blocking"));
    assert.equal(validatePptContentDraft(revoked).valid, false);
});

test("SHA-30c：公理层不受理念偏离影响——已承接偏离的块仍须有合法来源", () => {
    const draft = normalizePptContentDraft(
        {
            brief: { audience: "第一次接触产品的人", goal: "理解产品价值", narrative: "从问题到价值" },
            pages: [
    ...
    const page = draft.pageSpecs[0];
    const acknowledged = acknowledgePptPrincipleDeviation(draft, page.pageId, "cover-extra-content", "2026-07-23T08:00:00.000Z");
    const tampered = structuredClone(acknowledged);
    const tamperedPage = tampered.pageSpecs[0];
    const extraBlock = tamperedPage.contentBlocks.find((block) => block.text === "另一句与标题核心信息都不同的正文");
    extraBlock.sourceRefIds = [];
    assert.ok(validatePptPageSpec(tamperedPage).some((issue) => issue.code === "invalid_content_provenance"));
});

test("SHA-30c：move_block 迁移块及来源，目标页 version 递增，原页恢复合法", () => {
    const material = ["PPT 工作台", "把内容规划连接到页面生成", "另一句与标题核心信息都不同的正文", "第一次接触产品的人", "理解产品价值", "从问题到价值", "正文页要点"].join("\n");
    const draft = normalizePptContentDraft(
        {
            brief: { audience: "第一次接触产品的人", goal: "理解产品价值", narrative: "从问题到价值" },
    ...
    const nextPage = draft.pageSpecs[1];
    const extraBlock = coverPage.contentBlocks.find((block) => block.text === "另一句与标题核心信息都不同的正文");
    const extraSourceRefId = extraBlock.sourceRefIds[0];
    const extraSourceRef = coverPage.sourceRefs.find((sourceRef) => sourceRef.id === extraSourceRefId);
    const action = { kind: "move_block", pageId: coverPage.pageId, blockId: extraBlock.id, targetPageId: nextPage.pageId };
    const preview = previewPptContentAction(draft, action);
    const next = applyPptContentAction(draft, preview);

    const nextCoverPage = next.pageSpecs.find((page) => page.pageId === coverPage.pageId);
    const nextTargetPage = next.pageSpecs.find((page) => page.pageId === nextPage.pageId);
    assert.equal(
        nextCoverPage.contentBlocks.some((block) => block.id === extraBlock.id),
        false,
    );
    const movedBlock = nextTargetPage.contentBlocks.find((block) => block.id === extraBlock.id);
    assert.ok(movedBlock);
    // 迁移块原本是 body（非 title/primary_claim），不触发 merge_pages 那套降级语义，kind 保持不变。
    assert.equal(movedBlock.kind, "body");
    assert.equal(movedBlock.text, extraBlock.text);
    assert.ok(nextTargetPage.sourceRefs.some((sourceRef) => sourceRef.id === extraSourceRefId && sourceRef.excerpt === extraSourceRef.excerpt));
    assert.equal(nextTargetPage.version, nextPage.version + 1);
    assert.equal(nextCoverPage.version, coverPage.version + 1);
    // 原页恢复合法：不再有理念问题卡或 invalid_cover 判决
    assert.equal(
        next.audit.issues.some((issue) => issue.pageIds.includes(coverPage.pageId) && (issue.code === "principle_question" || issue.code === "invalid_cover")),
        false,
    );
    assert.deepEqual(validatePptPageSpec({ ...nextCoverPage, contentState: { status: "approved", approvedAt: "test" } }), []);
});

test("SHA-30c：move_block 迁移 title/primary_claim 块时降级为 supporting_claim（复用 merge_pages 语义）", () => {
    ...
    const titleBlock = twoPages.pageSpecs[0].contentBlocks.find((block) => block.kind === "title");
    const moved = applyPptContentAction(twoPages, previewPptContentAction(twoPages, { kind: "move_block", pageId: firstId, blockId: titleBlock.id, targetPageId: secondId }));
    const movedBlock = moved.pageSpecs.find((page) => page.pageId === secondId).contentBlocks.find((block) => block.id === titleBlock.id);
    assert.equal(movedBlock.kind, "supporting_claim");
    assert.equal(movedBlock.text, titleBlock.text);
});

test("SHA-30c：remove_block 删除内容块后原页恢复合法，标题与核心信息不可删除", () => {
    const draft = normalizePptContentDraft(
        {
            brief: { audience: "第一次接触产品的人", goal: "理解产品价值", narrative: "从问题到价值" },
            pages: [
    ...
    const page = draft.pageSpecs[0];
    const extraBlock = page.contentBlocks.find((block) => block.text === "另一句与标题核心信息都不同的正文");
    const titleBlock = page.contentBlocks.find((block) => block.kind === "title");
    assert.throws(() => previewPptContentAction(draft, { kind: "remove_block", pageId: page.pageId, blockId: titleBlock.id }), /标题和核心信息不能删除/);

    const next = applyPptContentAction(draft, previewPptContentAction(draft, { kind: "remove_block", pageId: page.pageId, blockId: extraBlock.id }));
    const nextPage = next.pageSpecs.find((item) => item.pageId === page.pageId);
    assert.equal(
        nextPage.contentBlocks.some((block) => block.id === extraBlock.id),
        false,
    );
    assert.equal(nextPage.version, page.version + 1);
    assert.equal(
        next.audit.issues.some((issue) => issue.pageIds.includes(page.pageId)),
        false,
    );
    assert.deepEqual(validatePptPageSpec({ ...nextPage, contentState: { status: "approved", approvedAt: "test" } }), []);
});

test("SHA-30c：单页重生成继承 previousPageSpec 的已承接理念偏离", () => {
    const draft = normalizePptContentDraft(
        {
            brief: { audience: "第一次接触产品的人", goal: "理解产品价值", narrative: "从问题到价值" },
            pages: [
    ...
    const regenerated = normalizePptContentDraft(
        ...
        },
        {
            ...
            sourceMaterial: ["PPT 工作台", "把内容规划连接到页面生成", "第一次接触产品的人", "理解产品价值", "从问题到价值", "另一句与标题核心信息都不同的正文"].join("\n"),
            requirements: "",
            previousPageSpecs: [acknowledgedPage],
        },
    );
    assert.deepEqual(regenerated.pageSpecs[0].principleDeviations, acknowledgedPage.principleDeviations);
    assert.equal(
        regenerated.audit.issues.some((issue) => issue.pageIds.includes(regenerated.pageSpecs[0].pageId)),
        false,
    );
});

test("SHA-30b：封面页缺口无绑定块时，采纳/回答不再追加正文块，gap resolution 语义不变", () => {
    const draft = normalizePptContentDraft(
        {
            brief: { audience: "第一次接触产品的人", goal: "理解产品价值", narrative: "从问题到价值" },
            pages: [
                {
                    ...
                    blocks: [],
                    layoutIntent: [],
                    gaps: [{ key: "tagline", kind: "missing_detail", question: "封面是否需要补充一句定位语？", reason: "材料未提供额外定位语", blocking: false, proposedAnswer: "让材料到可交付 PPT 之间不再有黑箱" }],
                },
            ],
        },
        {
    ...
    const gap = draft.audit.gaps.find((item) => item.question === "封面是否需要补充一句定位语？");
    const resolved = resolvePptInformationGap(draft, gap.id, { kind: "confirmed_assumption", text: gap.proposedAnswer, resolvedAt: "2026-07-22T10:00:00.000Z" });
    const page = resolved.pageSpecs[0];
    assert.deepEqual(
        page.contentBlocks.map((block) => block.kind),
        ["title", "primary_claim"],
    );
    const resolvedGap = resolved.audit.gaps.find((item) => item.id === gap.id);
    assert.equal(resolvedGap.resolution?.kind, "confirmed_assumption");
    assert.equal(resolvedGap.resolution?.text, gap.proposedAnswer);
});

test("SHA-30b：重复解析同一页时 autoTidy 重建而非累加", () => {
    const brief = { audience: "第一次接触产品的人", goal: "理解产品价值", narrative: "从问题到价值" };
    const input = {
        title: "PPT 工作台",
        sourceMaterial: ["PPT 工作台", "把内容规划连接到页面生成", "第一次接触产品的人", "理解产品价值", "从问题到价值"].join("\n"),
        requirements: "",
    ...
    const rawPage = {
        ...
        contentForm: "cover",
        blocks: [{ key: "echo", kind: "body", text: "把内容规划连接到页面生成", source: { source: "material", startLine: 2, endLine: 2 } }],
        layoutIntent: ["瀑布式·居中布局"],
        visualEncoding: [],
        gaps: [],
    };
    const first = normalizePptContentDraft({ brief, pages: [structuredClone(rawPage)] }, input);
    assert.equal(first.pageSpecs[0].autoTidy?.length, 2);

    const second = normalizePptContentDraft({ brief, pages: [structuredClone(rawPage)] }, { ...input, previousPageSpecs: [first.pageSpecs[0]] });
    assert.equal(second.pageSpecs[0].autoTidy?.length, 2);
});

test("SHA-30b：previousMatch 复用的已确认块与标题/核心信息同文时也会被清掉，确认状态不会尸变", () => {
    const brief = { audience: "第一次接触产品的人", goal: "理解产品价值", narrative: "从问题到价值" };
    const input = {
        title: "PPT 工作台",
        sourceMaterial: ["PPT 工作台", "先聚焦定位，再展开细节", "第一次接触产品的人", "理解产品价值", "从问题到价值"].join("\n"),
        requirements: "",
    };
    const initial = normalizePptContentDraft(
        {
            brief,
            pages: [
                {
                    ...
                    titleSource: { source: "material", startLine: 1, endLine: 1 },
                    purpose: "建立产品定位",
                    primaryClaim: "先聚焦定位，再展开细节",
                    primaryClaimSource: { source: "material", startLine: 2, endLine: 2 },
                    contentForm: "cover",
                    blocks: [{ key: "extra", kind: "placeholder", text: "待补充", gapKey: "extra" }],
                    layoutIntent: [],
                    gaps: [{ key: "extra", kind: "missing_detail", question: "封面是否需要补充一句话？", reason: "材料未提供", blocking: true, proposedAnswer: "先聚焦定位，再展开细节" }],
                },
            ],
        },
        input,
    );
    const gap = initial.audit.gaps.find((item) => item.question === "封面是否需要补充一句话？");
    const confirmed = resolvePptInformationGap(initial, gap.id, { kind: "confirmed_assumption", text: gap.proposedAnswer, resolvedAt: "2026-07-22T10:00:00.000Z" });
    const previousPage = confirmed.pageSpecs[0];
    assert.ok(previousPage.contentBlocks.some((block) => block.gapId === gap.id));

    // 重跑解析：raw 声明的正文块与已确认块同文，previousMatch 会复用其身份/来源——但它现在与核心信息同文，触发封面清理。
    const regenerated = normalizePptContentDraft(
        {
            brief,
            pages: [
                {
                    ...
                    primaryClaimSource: { source: "material", startLine: 2, endLine: 2 },
                    contentForm: "cover",
                    blocks: [{ key: "extra", kind: "body", text: "先聚焦定位，再展开细节", gapKey: "extra" }],
                    layoutIntent: [],
                    gaps: [{ key: "extra", kind: "missing_detail", question: "封面是否需要补充一句话？", reason: "材料未提供", blocking: true, proposedAnswer: "先聚焦定位，再展开细节" }],
                },
            ],
        },
        { ...input, previousPageSpecs: [previousPage] },
    );
    const page = regenerated.pageSpecs[0];
    assert.deepEqual(
        page.contentBlocks.map((block) => block.kind),
        ["title", "primary_claim"],
    );
    // 没有孤儿 sourceRef 残留，也没有因删块而复活一个新的未解决 gap
    assert.deepEqual(page.sourceRefs.map((source) => source.id).sort(), page.contentBlocks.flatMap((block) => block.sourceRefIds).sort());
    assert.equal(
        regenerated.audit.gaps.some((item) => item.pageId === page.pageId),
        false,
    );
    assert.ok(page.autoTidy?.length);
});
...
test("SHA-30a：布局词表扩充后，「上下主次/居中聚焦/步骤堆叠」等纯排版描述不再被误报", () => {
    const draft = normalizePptContentDraft(
        {
            brief: rawDraft().brief,
            pages: [{ ...rawDraft().pages[0], gaps: [], blocks: rawDraft().pages[0].blocks.filter((block) => block.key === "known"), visualEncoding: [], layoutIntent: [] }],
        },
        sourceInput(),
    );
    const page = draft.pageSpecs[0];

    // 词表覆盖后无需借助本页正文碰巧包含这些字词即可通过。
    assert.equal(isPptLayoutIntentSupported(page, "上下主次"), true);
    assert.equal(isPptLayoutIntentSupported(page, "居中聚焦"), true);
    assert.equal(isPptLayoutIntentSupported(page, "步骤堆叠"), true);
    assert.equal(isPptLayoutIntentSupported(page, "层次、错落、韵律、呼吸、密度、对称、平衡、放射、环绕、阶梯"), true);

    // 新增词汇不放松硬事实校验：新词与数字/百分比混用时仍按既有兜底逻辑拦截。
    assert.equal(isPptLayoutIntentSupported(page, "比例 3:2"), true);
    assert.equal(isPptLayoutIntentSupported(page, "比例提升至 45%"), false);

    // 「垂直流程」中的「流程」不在词表内，维持原有语义：只有在本页已批准内容中出现才放行。
    const flowRaw = rawDraft();
    flowRaw.pages[0].gaps = [];
    flowRaw.pages[0].blocks = [{ key: "flow", kind: "body", text: "分四个阶段推进的实施流程", source: { source: "material", startLine: 3, endLine: 3 } }];
    flowRaw.pages[0].visualEncoding = [];
    flowRaw.pages[0].layoutIntent = [];
    const flowSourceInput = { ...sourceInput(), sourceMaterial: "中转站介绍\n梳理思路、招募伙伴并展示未来空间\n分四个阶段推进的实施流程" };
    const flowPage = normalizePptContentDraft(flowRaw, flowSourceInput).pageSpecs[0];
    assert.equal(isPptLayoutIntentSupported(flowPage, "垂直流程 · 步骤堆叠"), true);
    assert.equal(isPptLayoutIntentSupported(page, "垂直流程 · 步骤堆叠"), false);
});

test("SHA-30a：残余按硬事实特征分级报错，纯修饰词不再指控用户夹带事实，硬事实残余文案不变", () => {
    const draft = normalizePptContentDraft(
        {
            brief: rawDraft().brief,
            pages: [
                {
                    ...rawDraft().pages[0],
                    gaps: [],
                    blocks: rawDraft().pages[0].blocks.filter((block) => block.key === "known"),
                    visualEncoding: [],
                    layoutIntent: ["深蓝科技风，螺旋式排列", "深蓝科技风，下方说明服务器投入 999 台"] /* SHA-30b 合入后:混入风格描述绕过构造期整理,校验用户路径的报错分级 */,
                },
            ],
        },
        sourceInput(),
    );
    const issues = draft.audit.issues.filter((issue) => issue.field === "layoutIntent");
    const modifierIssue = issues.find((issue) => issue.value === "深蓝科技风，螺旋式排列");
    const factRiskIssue = issues.find((issue) => issue.value === "深蓝科技风，下方说明服务器投入 999 台");

    assert.ok(modifierIssue);
    assert.equal(modifierIssue.message, "排版词「螺旋式」不在识别词表中，也未出现在本页已批准内容里");
    assert.equal(modifierIssue.severity, "blocking");
    assert.equal(modifierIssue.repair?.kind, "remove_layout_intent");

    assert.ok(factRiskIssue);
    assert.equal(factRiskIssue.message, "无法识别排版要求「深蓝科技风，下方说明服务器投入 999 台」；其中可能包含未批准的文案或事实");
    assert.equal(factRiskIssue.severity, "blocking");
});
...
test("无法识别的布局指出原值，并可按 revision 只移除目标布局", () => {
    ...
    raw.pages[0].gaps = [];
    raw.pages[0].visualEncoding = [];
    // SHA-30b：纯布局的未识别残余已在构造期被静默整理（见「初稿 layoutIntent 里的事实/未识别残余…」用例）；
    // 这里混入无法安全分离的风格描述以绕过构造期整理，保留 remove_layout_intent 修复回归。
    raw.pages[0].layoutIntent = ["左右分栏", "深蓝科技风，四宫格展示腾讯云、阿里云"];
    const draft = normalizePptContentDraft(raw, sourceInput());
    const issue = draft.audit.issues.find((item) => item.field === "layoutIntent");

    assert.ok(issue);
    assert.equal(issue.value, "深蓝科技风，四宫格展示腾讯云、阿里云");
    // SHA-30a：残余「腾讯云、阿里云」不含数字/引号/大写术语等硬事实特征，按纯修饰词分级，文案不再复述整条判决句。
    assert.match(issue.message, /排版词「腾讯云、阿里云」不在识别词表中，也未出现在本页已批准内容里/);
...
test("多页方案的第一页必须是无正文块封面，不能按页序静默改角色", () => {
    ...
    // SHA-30c：封面承载真实内容块不再判为 invalid_cover，而是带选项的 principle_question 问题卡（仍 blocking，直到用户承接或处理）。
    ...
    assert.ok(bodyCover.audit.issues.some((item) => item.code === "principle_question" && item.severity === "blocking"));
    ...
    // SHA-32：复核只判请求项，此处显式请求 invalid_cover 才能验证「仍未解决」拒绝。
    assert.throws(() => assertPptPageAuditIssuesResolved(regenerated, secondCover.pageId, [{ code: "invalid_cover", field: "contentForm", message: "第二页不能是封面" }]), /问题仍未解决|原页已保留/);
...
test("封面核心信息必须是一句定位语，不能用四项目标清单冒充", () => {
    ...
    // SHA-30c：目标清单式核心信息不再判为 invalid_cover，而是带选项的 principle_question 问题卡。
    const checklist = createCover("为什么需要、好在哪里、解决什么问题、为谁服务");
    assert.ok(checklist.audit.issues.some((item) => item.code === "principle_question" && item.field === "primaryClaim"));
...
test("SHA-21：不合格封面拒绝替换，合格封面可通过", () => {
    ...
    // SHA-30c：封面理念违规（额外内容块 + 目标清单式核心信息）不再判为 invalid_cover，而是带选项的 principle_question 问题卡；
    // 该卡不再走通用「修复本页」CTA（pptPageRepairActionLabel 对未知 code 落回默认值），改由问题卡自身的承接/迁移/删除选项处理。
    const current = normalizePptContentDraft(badCoverRaw, input);
    const coverIssues = current.audit.issues.filter((issue) => issue.code === "principle_question" && issue.pageIds.includes(current.pageSpecs[0].pageId));
    assert.equal(coverIssues.length, 2);
    assert.equal(pptPageRepairActionLabel(coverIssues[0]), "修复本页");

    const requested = selectPptPageRepairAuditIssues(current, current.pageSpecs[0].pageId, coverIssues[0].id);
    assert.ok(requested.some((issue) => issue.code === "principle_question"));
    ...
    assert.doesNotThrow(() => assertPptPageAuditIssuesResolved(accepted, current.pageSpecs[0].pageId, requested));
    assert.equal(
        accepted.audit.issues.some((issue) => issue.code === "principle_question" && issue.pageIds.includes(current.pageSpecs[0].pageId)),
        false,
    );
});
...
// --- SHA-31 / SHA-32 缺口性 provenance ---

test("SHA-32：缺口性 provenance 判定——纯来源合法返回 null，无来源但没有对应缺口时也返回 null", () => {
    const material = ["PPT 工作台", "把内容规划连接到页面生成", "另一句与标题核心信息都不同的正文", "第一次接触产品的人", "理解产品价值", "从问题到价值"].join("\n");
    const draft = normalizePptContentDraft(
        {
            brief: { audience: "第一次接触产品的人", goal: "理解产品价值", narrative: "从问题到价值" },
            pages: [
    ...
    const page = draft.pageSpecs[0];
    const pageGaps = draft.audit.gaps.filter((gap) => gap.pageId === page.pageId);
    assert.equal(derivePptPageProvenanceGapCause(page, pageGaps), null, "所有块都有合法来源，不存在 provenance 问题");

    const tampered = structuredClone(page);
    const extraBlock = tampered.contentBlocks.find((block) => block.text === "另一句与标题核心信息都不同的正文");
    extraBlock.sourceRefIds = []; // 人为造成无来源但没有对应缺口——非缺口性原因，不能算缺口引起
    assert.equal(derivePptPageProvenanceGapCause(tampered, pageGaps), null, "无来源但没有对应未决缺口时不能判定为缺口性");
});

test("SHA-32：无来源的正文块对应未决 unsupported_claim 缺口时判定为缺口性，且被排除出修复请求", () => {
    const unsupportedBody = "陌".repeat(300);
    const raw = {
        brief: { audience: "伙伴", goal: "理解", narrative: "主线" },
        pages: [
            {
                ...
                primaryClaimSource: { source: "material", startLine: 2, endLine: 2 },
                contentForm: "narrative",
                blocks: [{ key: "b", kind: "body", text: unsupportedBody }],
                layoutIntent: ["上下分区"],
                visualEncoding: [],
                gaps: [],
            },
    ...
    const input = { title: "页", sourceMaterial: "页\n主张\n伙伴\n理解\n主线", requirements: "" };
    const draft = normalizePptContentDraft(raw, input);
    const pageId = draft.pageSpecs[0].pageId;

    const provenanceIssue = draft.audit.issues.find((issue) => issue.code === "invalid_content_provenance" && issue.pageIds.includes(pageId));
    assert.ok(provenanceIssue, "应检出无来源的正文块");
    const excessiveIssue = draft.audit.issues.find((issue) => issue.code === "excessive_copy" && issue.pageIds.includes(pageId));
    assert.ok(excessiveIssue, "应检出超长文案");

    const gapCause = derivePptPageProvenanceGapCause(
        draft.pageSpecs[0],
        draft.audit.gaps.filter((gap) => gap.pageId === pageId),
    );
    assert.ok(gapCause, "应判定为缺口性 provenance");
    assert.equal(gapCause.gapIds.length, 1);

    const requested = selectPptPageRepairAuditIssues(draft, pageId);
    assert.equal(
        requested.some((issue) => issue.code === "invalid_content_provenance"),
        false,
        "缺口性 provenance 项不应进入修复请求",
    );
    assert.ok(
        requested.some((issue) => issue.code === "excessive_copy"),
        "非缺口性问题应保留在修复请求中",
    );
});

test("SHA-32：请求项全部解决即可采纳，即使页面仍残留缺口性 provenance", () => {
    const rawPage = {
        title: "页",
        titleSource: { source: "material", startLine: 1, endLine: 1 },
        purpose: "说明",
        ...
        primaryClaimSource: { source: "material", startLine: 2, endLine: 2 },
        contentForm: "narrative",
        blocks: [{ key: "b", kind: "body", text: "陌".repeat(300) }],
        layoutIntent: ["上下分区"],
        visualEncoding: [],
        gaps: [],
    };
    const input = { title: "页", sourceMaterial: "页\n主张\n伙伴\n理解\n主线", requirements: "" };
    const current = normalizePptContentDraft({ brief: { audience: "伙伴", goal: "理解", narrative: "主线" }, pages: [rawPage] }, input);
    const pageId = current.pageSpecs[0].pageId;
    const excessive = current.audit.issues.find((issue) => issue.code === "excessive_copy" && issue.pageIds.includes(pageId));
    assert.ok(excessive);

    const requested = selectPptPageRepairAuditIssues(current, pageId, excessive.id);
    assert.equal(
        requested.some((issue) => issue.code === "invalid_content_provenance"),
        false,
    );

    const shortRaw = structuredClone(rawPage);
    shortRaw.blocks[0].text = "全新的简短表述";
    const regenerated = normalizePptContentDraft({ brief: current.brief, pages: [shortRaw] }, { ...input, previousPageSpecs: current.pageSpecs });
    const next = replacePptContentDraftPage(
        current,
        current.revision,
        pageId,
        regenerated.pageSpecs[0],
        regenerated.audit.gaps.filter((gap) => gap.pageId === pageId),
    );

    assert.equal(
        next.audit.issues.some((issue) => issue.code === "excessive_copy" && issue.pageIds.includes(pageId)),
        false,
    );
    assert.ok(
        next.audit.issues.some((issue) => issue.code === "invalid_content_provenance" && issue.pageIds.includes(pageId)),
        "缺口性 provenance 仍应残留",
    );
    assert.ok(next.audit.gaps.some((gap) => gap.pageId === pageId && !gap.resolution && gap.kind === "unsupported_claim"));

    assert.doesNotThrow(() => assertPptPageAuditIssuesResolved(next, pageId, requested));
});

test("SHA-32：请求项未全部解决时仍拒绝，报错文案只列未解决的请求项", () => {
    const rawPage = {
        title: "页",
        titleSource: { source: "material", startLine: 1, endLine: 1 },
        purpose: "说明",
    ...
    const unchanged = normalizePptContentDraft(structuredClone({ brief: current.brief, pages: [rawPage] }), { ...input, previousPageSpecs: current.pageSpecs });
    const rejected = replacePptContentDraftPage(
        current,
        current.revision,
        pageId,
        unchanged.pageSpecs[0],
        unchanged.audit.gaps.filter((gap) => gap.pageId === pageId),
    );

    assert.throws(() => {
        try {
            assertPptPageAuditIssuesResolved(rejected, pageId, requested);
        } catch (error) {
            assert.match(error.message, /excessive_copy|字|压缩|拆页/);
            assert.doesNotMatch(error.message, /页面内容存在缺失或无效的来源/);
            throw error;
        }
    }, /问题仍未解决|原页已保留/);
});
...
test("SHA-27：多实质块绑定同 gap 采纳后文本不变且挂确认 sourceRef", () => {
    ...
    assert.equal(page.contentBlocks.filter((block) => block.text === answer).length, 0);
