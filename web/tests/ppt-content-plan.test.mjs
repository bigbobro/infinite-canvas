...
test("SHA-30b：封面不同文的真实内容块仍保留现状，照常报既有 issue（SHA-30c 接手前行为不变）", () => {
    const draft = normalizePptContentDraft(
        {
            brief: { audience: "第一次接触产品的人", goal: "理解产品价值", narrative: "从问题到价值" },
            pages: [
    ...
    assert.ok(draft.audit.issues.some((issue) => issue.code === "invalid_cover" && issue.pageIds.includes(page.pageId)));
});
...
test("SHA-30a：残余按硬事实特征分级报错，纯修饰词不再指控用户夹带事实，硬事实残余文案不变", () => {
    const draft = normalizePptContentDraft(
        {
            brief: rawDraft().brief,
            pages: [{ ...rawDraft().pages[0], gaps: [], blocks: rawDraft().pages[0].blocks.filter((block) => block.key === "known"), visualEncoding: [], layoutIntent: ["深蓝科技风，螺旋式排列", "深蓝科技风，下方说明服务器投入 999 台"] /* SHA-30b 合入后:混入风格描述绕过构造期整理,校验用户路径的报错分级 */ }],
        },
        sourceInput(),
    );
