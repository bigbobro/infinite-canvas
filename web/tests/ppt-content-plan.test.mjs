...
test("SHA-30b：词表外的纯修饰词残余在构造期被剥除，保留可识别部分并留痕", () => {
    ...
    raw.pages[0].gaps = [];
    raw.pages[0].visualEncoding = [];
    raw.pages[0].layoutIntent = ["上下主次·居中布局"];
    const draft = normalizePptContentDraft(raw, sourceInput());
...
test("SHA-30a：残余按硬事实特征分级报错，纯修饰词不再指控用户夹带事实，硬事实残余文案不变", () => {
    const draft = normalizePptContentDraft(
        {
            brief: rawDraft().brief,
            pages: [{ ...rawDraft().pages[0], gaps: [], blocks: rawDraft().pages[0].blocks.filter((block) => block.key === "known"), visualEncoding: [], layoutIntent: ["螺旋式排列", "下方说明服务器投入 999 台"] }],
        },
        sourceInput(),
    );
    const issues = draft.audit.issues.filter((issue) => issue.field === "layoutIntent");
    const modifierIssue = issues.find((issue) => issue.value === "螺旋式排列");
    const factRiskIssue = issues.find((issue) => issue.value === "下方说明服务器投入 999 台");
    ...
    assert.equal(factRiskIssue.message, "无法识别排版要求「下方说明服务器投入 999 台」；其中可能包含未批准的文案或事实");
