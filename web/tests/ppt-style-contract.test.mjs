...
test("presetId 只记录来源，相同冻结 direction 编译出相同最终提示词", () => {
    const build = (presetId) => compilerModel([], { source: { kind: "preset", presetId }, direction: "同一份冻结视觉方向", references: [] });
...
test("生成计划忽略 style 投影节点，Contract 参考图进入快照但不伪造 inputRefs", () => {
    const { deckBrief, pageSpecs } = compilerModel([{ storageKey: "image:contract-reference" }], { source: { kind: "custom" }, direction: "Contract 唯一方向", references: [{ storageKey: "image:contract-reference" }] });
    const partial = buildPptDeckProject({
        compilePolicy: "structured",
        title: "单一视觉事实源",
        sourceMaterial: "项目结论",
...
function compilerModel(references = [], contract = { source: { kind: "custom" }, direction: "深蓝、克制、专业", references }, sourceMaterial = "项目结论") {
    return {
...
function deckBrief(styleContract, sourceMaterial = "", requirements = "") {
    return { version: 1, sourceHash: hashPptContentSource(sourceMaterial, requirements), audience: "", goal: "", narrative: "", styleContract: structuredClone(styleContract), globalRules: [], forbiddenRules: [], lockedDeckFacts: [] };
}
