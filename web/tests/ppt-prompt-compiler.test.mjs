...
test("Compiler 把已批准文案按语义块编译，并允许无文字视觉构件", () => {
    ...
    assert.match(prompt, /允许新增不含文字的形状、连线、分区和图表容器/);
    assert.match(prompt, /图标只在信息表达明确要求时使用，不为要点列表和副标题逐条配图标/);
    assert.match(prompt, /条目层级改用编号、字重和留白表达/);
    assert.doesNotMatch(prompt, /允许新增不含文字的图标、形状、连线/);
    assert.match(prompt, /分区大小与位置须反映内容主次，不得等宽等高平均分格；每页确立一个视觉重心/);
    ...
    assert.match(comparisonPrompt, /对比或数据关系所需的等宽等高分栏按内容语义保留，不受主次分区约束/);
    assert.doesNotMatch(comparisonPrompt, /分区大小与位置须反映内容主次/);
    assert.match(comparisonPrompt, /【整套视觉系统】/);
});

test("SHA-37：icon 默认克制，声明 visualEncoding channel:icon 时显式指令与克制条款并存", () => {
    const text = ["核心能力概览", "接入、路由与治理三项能力共同支撑稳定运行", "接入能力：兼容主流服务商协议", "路由能力：并发调度与故障切换", "治理能力：密钥、鉴权与审计闭环"].join("\n");
    const { deckBrief, pageSpecs } = singlePageModel({ pageId: "page-icon-encoding", title: "核心能力概览", text, styleContract: styleContract() });

    const withoutIconEncoding = compilePptPromptSnapshot(snapshotInputForPage(deckBrief, pageSpecs[0], "take-no-icon")).prompts[0].finalPrompt;
    assert.match(withoutIconEncoding, /图标只在信息表达明确要求时使用/);
    assert.doesNotMatch(withoutIconEncoding, /对 B\d+ 使用图标/);

    const withIconEncoding = structuredClone(pageSpecs[0]);
    withIconEncoding.visualEncoding = [
        {
            id: "page-icon-encoding:encoding:1",
            contentBlockIds: [withIconEncoding.contentBlocks[2].id],
            intent: "differentiate",
            channel: "icon",
        },
    ];
    const withIconPrompt = compilePptPromptSnapshot(snapshotInputForPage(deckBrief, withIconEncoding, "take-with-icon")).prompts[0].finalPrompt;
    assert.match(withIconPrompt, /对 B3 使用图标区分内容类别/);
    assert.match(withIconPrompt, /图标只在信息表达明确要求时使用，不为要点列表和副标题逐条配图标/);
});

test("SHA-37：architecture/process 页的结构示意图节点符号不受图标限制豁免", () => {
    const { deckBrief, pageSpecs } = singlePageModel({ pageId: "page-architecture", title: "系统架构", text: "系统架构\n接入层、路由层与存储层通过内部总线连接", styleContract: styleContract() });

    const architecture = structuredClone(pageSpecs[0]);
    architecture.contentForm = "architecture";
    const architecturePrompt = compilePptPromptSnapshot(snapshotInputForPage(deckBrief, architecture, "take-architecture")).prompts[0].finalPrompt;
    assert.match(architecturePrompt, /结构示意图和流程图中的节点图形符号不受此限/);

    const process = structuredClone(pageSpecs[0]);
    process.contentForm = "process";
    const processPrompt = compilePptPromptSnapshot(snapshotInputForPage(deckBrief, process, "take-process")).prompts[0].finalPrompt;
    assert.match(processPrompt, /结构示意图和流程图中的节点图形符号不受此限/);

    const narrative = structuredClone(pageSpecs[0]);
    narrative.contentForm = "narrative";
    const narrativePrompt = compilePptPromptSnapshot(snapshotInputForPage(deckBrief, narrative, "take-narrative")).prompts[0].finalPrompt;
    assert.doesNotMatch(narrativePrompt, /结构示意图和流程图中的节点图形符号不受此限/);
});

test("SHA-37：data 页与 comparison 页同享等宽等高分栏豁免", () => {
    const { deckBrief, pageSpecs } = singlePageModel({ pageId: "page-data-split", title: "关键指标对照", text: "关键指标对照\n设备在线率 98.5%", styleContract: styleContract() });
    const data = structuredClone(pageSpecs[0]);
    data.contentForm = "data";
    const dataPrompt = compilePptPromptSnapshot(snapshotInputForPage(deckBrief, data, "take-data-split")).prompts[0].finalPrompt;
    assert.match(dataPrompt, /对比或数据关系所需的等宽等高分栏按内容语义保留，不受主次分区约束/);
    assert.doesNotMatch(dataPrompt, /分区大小与位置须反映内容主次/);
});
...
test("SHA-29：多页编译注入各自页码与章节标签，shell 全 none 时不注入", () => {
    const sectionTitle = "登录与安全";
    const pageSpecs = [
        createPageSpec({ pageId: "page-cover", title: "封面", text: "封面\n整套开场", sourceKind: "confirmed_assumption", layoutRole: "cover", contentForm: "cover" }),
        createPageSpec({ pageId: "page-section", title: sectionTitle, text: `${sectionTitle}\n本章要点`, sourceKind: "confirmed_assumption", layoutRole: "section", contentForm: "narrative" }),
        createPageSpec({ pageId: "page-content", title: "内容页", text: "内容页\n具体方案", sourceKind: "confirmed_assumption", layoutRole: "content", contentForm: "narrative" }),
        createPageSpec({ pageId: "page-close", title: "收尾", text: "收尾\n行动号召", sourceKind: "confirmed_assumption", layoutRole: "close", contentForm: "closing" }),
    ];
    const deckBrief = {
        version: 1,
        sourceHash: hashPptContentSource("封面", ""),
        contentRevision: `${hashPptContentSource("封面", "")}:r1`,
        audience: "决策者",
        goal: "统一理解",
        narrative: "从开场到行动",
        styleContract: createPptVisualDirectionPresetContract("clean-report"),
        globalRules: [],
        forbiddenRules: [...createPptVisualDirectionPresetContract("clean-report").modelStyle.forbiddenRules],
        lockedDeckFacts: [],
    };
    const deckTitle = "3X-UI 方案";
    const deckShell = derivePptDeckShellFacts(pageSpecs, deckTitle);
    assert.deepEqual(
        deckShell.pages.map((page) => ({ pageId: page.pageId, pageNumber: page.pageNumber, sectionLabel: page.sectionLabel })),
        [
            { pageId: "page-cover", pageNumber: 1, sectionLabel: undefined },
            { pageId: "page-section", pageNumber: 2, sectionLabel: `第 1 章 · ${sectionTitle}` },
            { pageId: "page-content", pageNumber: 3, sectionLabel: `第 1 章 · ${sectionTitle}` },
            { pageId: "page-close", pageNumber: 4, sectionLabel: undefined },
        ],
    );

    const snapshot = compilePptPromptSnapshot({
        compilePolicy: "structured",
        snapshotId: "snapshot-sha29-shell",
        compiledAt: "2026-07-23T08:00:00.000Z",
        deckBrief,
        pageSpecs,
        deckShell,
        targets: pageSpecs.map((pageSpec, index) => ({
            pageId: pageSpec.pageId,
            takeId: `take-${index + 1}`,
            semanticText: renderPptPageSpecText(pageSpec),
            layoutIntent: [],
    ...
    for (const [index, pageSpec] of pageSpecs.entries()) {
        const prompt = promptFor(snapshot, pageSpec.pageId).finalPrompt;
        assert.match(prompt, new RegExp(`本页页码 ${index + 1}，总页数 4；页脚页码必须显示为 ${index + 1}/4`));
        assert.match(prompt, new RegExp(`整套标题：${deckTitle}`));
        assert.match(prompt, /参考图仅用于对齐配色、字体、图形语言与外壳位置/);
        assert.match(prompt, /不得复制参考图或其他页面的正文构图/);
        if (pageSpec.pageId === "page-section" || pageSpec.pageId === "page-content") {
            assert.match(prompt, new RegExp(`本页章节标签：第 1 章 · ${sectionTitle}`));
        } else {
            assert.doesNotMatch(prompt, /本页章节标签：/);
            assert.doesNotMatch(prompt, /本套无章节分组/);
        }
    }

    const noSectionSpecs = [
        createPageSpec({ pageId: "page-a", title: "A", text: "A\n甲", sourceKind: "confirmed_assumption", layoutRole: "content" }),
        createPageSpec({ pageId: "page-b", title: "B", text: "B\n乙", sourceKind: "confirmed_assumption", layoutRole: "content" }),
    ];
    const noSectionSnapshot = compilePptPromptSnapshot({
        compilePolicy: "structured",
        snapshotId: "snapshot-sha29-no-section",
        compiledAt: "2026-07-23T08:00:00.000Z",
        deckBrief,
        pageSpecs: noSectionSpecs,
        deckShell: derivePptDeckShellFacts(noSectionSpecs, deckTitle),
        targets: noSectionSpecs.map((pageSpec, index) => ({
            pageId: pageSpec.pageId,
            takeId: `take-ns-${index + 1}`,
            semanticText: renderPptPageSpecText(pageSpec),
            layoutIntent: [],
            layoutConfirmed: true,
            extraTexts: [],
    ...
    assert.equal(hasBlockingCompilationIssues(noSectionSnapshot), false);
    for (const prompt of noSectionSnapshot.prompts) {
        assert.doesNotMatch(prompt.finalPrompt, /第 \d+ 章/);
        assert.match(prompt.finalPrompt, /本套无章节分组，页眉不得出现章节编号/);
    }

    const noneShellBrief = structuredClone(deckBrief);
    noneShellBrief.styleContract.modelStyle.shell = { safeArea: "regular", titleRegion: "top-left", header: "none", footer: "none" };
    const noneShellSnapshot = compilePptPromptSnapshot({
        compilePolicy: "structured",
        snapshotId: "snapshot-sha29-shell-none",
        compiledAt: "2026-07-23T08:00:00.000Z",
        deckBrief: noneShellBrief,
        pageSpecs: noSectionSpecs,
        deckShell: derivePptDeckShellFacts(noSectionSpecs, deckTitle),
        targets: noSectionSpecs.map((pageSpec, index) => ({
            pageId: pageSpec.pageId,
            takeId: `take-none-${index + 1}`,
            semanticText: renderPptPageSpecText(pageSpec),
            layoutIntent: [],
            layoutConfirmed: true,
            extraTexts: [],
    ...
    for (const prompt of noneShellSnapshot.prompts) {
        assert.doesNotMatch(prompt.finalPrompt, /【本页页面事实】/);
        assert.doesNotMatch(prompt.finalPrompt, /本页页码/);
        assert.doesNotMatch(prompt.finalPrompt, /参考图仅用于对齐配色/);
    }
});

test("SHA-29：override 丢掉页码时被 missing_required_instruction 阻断", () => {
    const { deckBrief, pageSpecs } = modelForFixturePages(["page-2"]);
    const input = snapshotInputForPage(deckBrief, pageSpecs[0], "take-shell-override", "工业智能方案");
    const baseline = compilePptPromptSnapshot(input).prompts[0].finalPrompt;
    assert.match(baseline, /本页页码 1，总页数 1/);
    const stripped = baseline
        .split("\n")
        .filter((line) => !line.includes("本页页码") && !line.includes("页脚页码必须显示为"))
        .join("\n");
    const snapshot = compilePptPromptSnapshot({
        ...input,
...
function snapshotInput(deckBrief, pageSpecs, deckTitle = "工业智能方案") {
    return {
        compilePolicy: "structured",
        ...
        deckBrief,
        pageSpecs,
        deckShell: derivePptDeckShellFacts(pageSpecs, deckTitle),
...
function snapshotInputForPage(deckBrief, pageSpec, takeId, deckTitle = "测试整套标题") {
    return {
        compilePolicy: "structured",
        ...
        deckBrief,
        pageSpecs: [pageSpec],
        deckShell: derivePptDeckShellFacts([pageSpec], deckTitle),
