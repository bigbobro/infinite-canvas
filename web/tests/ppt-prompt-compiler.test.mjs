...
test("5 点页的 override 新增第 6 点时阻断生成", () => {
    ...
    const snapshot = compilePptPromptSnapshot({
        ...snapshotInput(deckBrief, pageSpecs),
        targets: [{ pageId: page.pageId, takeId: page.takeId, semanticText: page.outline, layoutIntent: [page.layoutIntent], extraTexts: [], override }],
    });
...
test("显式 override 不能删除全局、禁止、布局或视觉约束", () => {
    ...
    const snapshot = compilePptPromptSnapshot({
        ...snapshotInput(deckBrief, pageSpecs),
        targets: [{ pageId: page.pageId, takeId: page.takeId, semanticText: page.outline, layoutIntent: [page.layoutIntent], extraTexts: [], override: page.outline }],
    });
...
test("显式 override 不能在完整编译结果后追加未确认事实", () => {
    ...
    const baseline = compilePptPromptSnapshot({
        ...snapshotInput(deckBrief, pageSpecs),
        targets: [{ pageId: page.pageId, takeId: page.takeId, semanticText: page.outline, layoutIntent: [page.layoutIntent], layoutConfirmed: true, extraTexts: [] }],
    });
    const snapshot = compilePptPromptSnapshot({
        ...snapshotInput(deckBrief, pageSpecs),
        targets: [{ pageId: page.pageId, takeId: page.takeId, semanticText: page.outline, layoutIntent: [page.layoutIntent], extraTexts: [], override: `${baseline.prompts[0].finalPrompt}\n未确认增长 99%` }],
    });
...
test("显式 override 不能追加不含数字的未确认结论", () => {
    ...
    const baseline = compilePptPromptSnapshot({
        ...snapshotInput(deckBrief, pageSpecs),
        targets: [{ pageId: page.pageId, takeId: page.takeId, semanticText: page.outline, layoutIntent: [page.layoutIntent], extraTexts: [] }],
    });
    const snapshot = compilePptPromptSnapshot({
        ...snapshotInput(deckBrief, pageSpecs),
        targets: [{ pageId: page.pageId, takeId: page.takeId, semanticText: page.outline, layoutIntent: [page.layoutIntent], extraTexts: [], override: `${baseline.prompts[0].finalPrompt}\n公司已经成为全球第一` }],
...
test("显式确认只对完整且未变更的 override 生效", () => {
    ...
    const confirmed = compilePptPromptSnapshot({
        ...snapshotInput(deckBrief, pageSpecs),
        targets: [{ pageId: page.pageId, takeId: page.takeId, semanticText: page.outline, layoutIntent: [page.layoutIntent], extraTexts: [], override, overrideConfirmed: true }],
    });
    assert.equal(
...
test("override 的删减、重排和 section 归属变化都必须重新确认", () => {
    const { deckBrief, pageSpecs } = buildModel();
    const page = fixture.pages[0];
    const target = { pageId: page.pageId, takeId: page.takeId, semanticText: page.outline, layoutIntent: [page.layoutIntent], extraTexts: [] };
...
test("自定义排版文本未显式确认时阻断", () => {
    ...
    const snapshot = compilePptPromptSnapshot({
        ...snapshotInput(deckBrief, pageSpecs),
        targets: [{ pageId: page.pageId, takeId: page.takeId, semanticText: page.outline, layoutIntent: ["用大号字体突出公司已经成为全球第一"], extraTexts: [] }],
    });
...
test("override 否定句子串和未知标题都需要显式确认", () => {
    ...
    const baseInput = {
        ...
        deckBrief,
        pageSpecs,
        targets: [{ pageId: "page-negative-copy", takeId: "take-negative-copy", semanticText: "公司不是全球第一", layoutIntent: [], extraTexts: [] }],
    };
...
test("未纳入 PageSpec 的额外文本输入会阻断生成", () => {
    ...
    const snapshot = compilePptPromptSnapshot({
        ...snapshotInput(deckBrief, pageSpecs),
        targets: [{ pageId: page.pageId, takeId: page.takeId, semanticText: page.outline, layoutIntent: [page.layoutIntent], extraTexts: ["公司已经成为全球第一"] }],
    });
...
test("未修改的自动编译结果保存为 override 不会误报点数", () => {
    ...
    const second = compilePptPromptSnapshot({
        ...snapshotInput(deckBrief, pageSpecs),
        targets: [{ pageId: page.pageId, takeId: page.takeId, semanticText: page.outline, layoutIntent: [page.layoutIntent], layoutConfirmed: true, extraTexts: [], override: first.prompts[0].finalPrompt }],
    });
