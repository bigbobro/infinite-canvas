...
test("SHA-33：三条嵌套/重叠区间合并为一条并集区间，摘要取最长者，支撑块数累加", () => {
    const page = {
        sourceRefs: [
            { id: "src-a", source: "material", relation: "verbatim", excerpt: "较短摘要", startLine: 197, endLine: 227 },
            { id: "src-b", source: "material", relation: "verbatim", excerpt: "这是三条重叠来源里最长的一份摘要文本", startLine: 199, endLine: 227 },
            { id: "src-c", source: "material", relation: "verbatim", excerpt: "中间嵌套区间", startLine: 215, endLine: 223 },
        ],
        contentBlocks: [
            { id: "block-1", kind: "supporting_claim", text: "块一", sourceRefIds: ["src-a"] },
            { id: "block-2", kind: "supporting_claim", text: "块二", sourceRefIds: ["src-b"] },
            { id: "block-3", kind: "supporting_claim", text: "块三", sourceRefIds: ["src-c"] },
        ],
    };
    const aggregated = aggregatePptSourceRefs(page);
    assert.equal(aggregated.length, 1);
    assert.equal(aggregated[0].startLine, 197);
    assert.equal(aggregated[0].endLine, 227);
    assert.equal(aggregated[0].excerpt, "这是三条重叠来源里最长的一份摘要文本");
    assert.equal(aggregated[0].supportedBlockCount, 3);
});

test("SHA-33：不相邻（不重叠、不接壤）区间不合并", () => {
    const page = {
        sourceRefs: [
            { id: "src-a", source: "material", relation: "verbatim", excerpt: "第一段落", startLine: 10, endLine: 15 },
            { id: "src-b", source: "material", relation: "verbatim", excerpt: "第二段落", startLine: 20, endLine: 25 },
        ],
        contentBlocks: [
            { id: "block-1", kind: "supporting_claim", text: "第一段落", sourceRefIds: ["src-a"] },
            { id: "block-2", kind: "supporting_claim", text: "第二段落", sourceRefIds: ["src-b"] },
        ],
    };
    const aggregated = aggregatePptSourceRefs(page);
    assert.equal(aggregated.length, 2);
    assert.equal(aggregated[0].startLine, 10);
    assert.equal(aggregated[0].endLine, 15);
    assert.equal(aggregated[1].startLine, 20);
    assert.equal(aggregated[1].endLine, 25);
});

test("SHA-33：不同 source/relation 即使区间重叠也不合并", () => {
    const page = {
        sourceRefs: [
            { id: "src-a", source: "material", relation: "verbatim", excerpt: "材料原文", startLine: 5, endLine: 8 },
            { id: "src-b", source: "requirements", relation: "verbatim", excerpt: "补充要求原文", startLine: 5, endLine: 8 },
            { id: "src-c", source: "material", relation: "derived", excerpt: "材料归纳", startLine: 5, endLine: 8 },
        ],
        contentBlocks: [
            { id: "block-1", kind: "supporting_claim", text: "材料原文", sourceRefIds: ["src-a"] },
            { id: "block-2", kind: "supporting_claim", text: "补充要求原文", sourceRefIds: ["src-b"] },
            { id: "block-3", kind: "supporting_claim", text: "材料归纳", sourceRefIds: ["src-c"] },
        ],
    };
    const aggregated = aggregatePptSourceRefs(page);
