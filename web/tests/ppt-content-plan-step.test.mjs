...
test("SHA-30b：有自动整理记录时显示留痕行，可展开逐条；无记录时不渲染", () => {
    const withTidy = renderPlan([], { autoTidy: ["已移除与核心信息重复的正文块", "排版表述「上下主次」已整体移除（均为未识别的排版修饰词）"] });
    assert.match(withTidy, /已自动整理 2 处/);
    assert.match(withTidy, /已移除与核心信息重复的正文块/);
    assert.match(withTidy, /排版表述「上下主次」已整体移除/);

    const withoutTidy = renderPlan([], {});
    assert.doesNotMatch(withoutTidy, /已自动整理/);
});

test("SHA-30c：cover-extra-content 理念问题渲染为问题卡，不进入通用「内容检查」列表", () => {
    const html = renderPlan([], {
        contentForm: "cover",
        contentBlocks: [
            { id: "title", kind: "title", text: "测试页", sourceRefIds: [] },
            { id: "claim", kind: "primary_claim", text: "核心信息", sourceRefIds: [] },
            { id: "extra", kind: "body", text: "额外的正文内容块", sourceRefIds: [] },
        ],
        issues: [
            {
                id: "issue-cover-extra",
                code: "principle_question",
                severity: "blocking",
                pageIds: ["page-1"],
                message: "封面只保留标题和一句定位语，不承载正文或目标清单",
                field: "contentForm",
                value: "cover",
                actions: [
                    { kind: "move_block", pageId: "page-1", blockId: "extra", targetPageId: "page-2" },
                    { kind: "remove_block", pageId: "page-1", blockId: "extra" },
                    { kind: "acknowledge_deviation", pageId: "page-1", principle: "cover-extra-content" },
                ],
            },
        ],
    });
    assert.match(html, /理念问题/);
    assert.match(html, /封面的理念是只留一句定位语，不承载正文内容/);
    assert.match(html, /额外的正文内容块/);
    assert.match(html, /移到下一页/);
    assert.match(html, /删除该块/);
    assert.match(html, /保留——我要这样（记入设计）/);
    assert.doesNotMatch(html, /内容检查/);
});

test("SHA-30c：cover-claim-checklist 理念问题卡提供改写与保留选项", () => {
    const html = renderPlan([], {
        contentForm: "cover",
        issues: [
            {
                id: "issue-cover-checklist",
                code: "principle_question",
                severity: "blocking",
                pageIds: ["page-1"],
                message: "封面核心信息应是一句定位语，不能复述整套目标或问题清单",
                field: "primaryClaim",
                value: "为什么需要、好在哪里",
                actions: [
                    { kind: "regenerate_pages", pageIds: ["page-1"] },
                    { kind: "acknowledge_deviation", pageId: "page-1", principle: "cover-claim-checklist" },
                ],
            },
        ],
    });
    assert.match(html, /封面核心信息的理念是一句定位语，不是目标或问题清单/);
    assert.match(html, /改写为一句定位语/);
    assert.match(html, /保留——我要这样（记入设计）/);
    assert.doesNotMatch(html, /移到下一页/);
});

test("SHA-30c：已承接的理念偏离原位显示可撤销的轻量行，且不再出现在问题卡里", () => {
    const html = renderPlan([], {
        contentForm: "cover",
        principleDeviations: [{ principle: "cover-extra-content", acknowledgedAt: "2026-07-23T00:00:00.000Z" }],
    });
    assert.match(html, /封面的理念是只留一句定位语，不承载正文内容/);
    assert.match(html, /已按你的设计保留/);
    assert.match(html, /撤销/);
    assert.doesNotMatch(html, /需要决定/);
});

test("SHA-30d：内容页职责仪表在页头正向展示，默认全部满足时不出现待办文案", () => {
    const html = renderPlan([]);
    assert.match(html, /支撑 0 条/);
    assert.match(html, /来源齐/);
    assert.doesNotMatch(html, /待补充/);
    assert.doesNotMatch(html, /待处理/);
});

test("SHA-30d：封面已承接理念偏离时仪表第二项联动显示按你的设计保留，而不是待办态", () => {
    const html = renderPlan([], {
        contentForm: "cover",
        principleDeviations: [{ principle: "cover-extra-content", acknowledgedAt: "2026-07-23T00:00:00.000Z" }],
    });
    // 定位语未偏离，仪表按 ok 展示 label；无多余承载已偏离，仪表只显示中性说明，不展示原 label
    assert.match(html, /定位语/);
    // 「已按你的设计保留」（撤销行）+ 仪表行各自一份「按你的设计保留」
    assert.equal(html.match(/按你的设计保留/g)?.length, 2);
});

test("SHA-30d：来源依据按区间聚合展示，同区间多块引用合并为一条并标注支撑块数", () => {
    const html = renderPlan([], {
        contentBlocks: [
            { id: "title", kind: "title", text: "测试页", sourceRefIds: [] },
            { id: "claim", kind: "primary_claim", text: "共享的核心表述", sourceRefIds: ["src-a"] },
            { id: "body", kind: "body", text: "共享的核心表述", sourceRefIds: ["src-b"] },
        ],
        sourceRefs: [
            { id: "src-a", source: "material", relation: "verbatim", excerpt: "共享的核心表述", startLine: 2, endLine: 2 },
            { id: "src-b", source: "material", relation: "verbatim", excerpt: "共享的核心表述", startLine: 2, endLine: 2 },
        ],
    });
    assert.match(html, /查看 1 条来源依据/);
    assert.match(html, /支撑 2 个内容块/);
});

test("SHA-30d：内容检查标题降为与来源依据同级的次要样式，不再使用警示色", () => {
    const html = renderPlan([], { issues: [issue("noise", "noise_text", "可能的异常文本")] });
    const labelIndex = html.indexOf("内容检查");
    assert.notEqual(labelIndex, -1);
    const tagStart = html.lastIndexOf("<p", labelIndex);
    const openingTag = html.slice(tagStart, html.indexOf(">", tagStart) + 1);
    assert.doesNotMatch(openingTag, /amber/);
});
...
function createPlanning(gaps, overrides = {}) {
    ...
    return {
            ...
            pageSpecs: [
                {
                    ...
                    layoutIntent: [],
                    visualEncoding: [],
                    ...(overrides.autoTidy ? { autoTidy: overrides.autoTidy } : {}),
                    ...(overrides.principleDeviations ? { principleDeviations: overrides.principleDeviations } : {}),
        ...
        moveBlock: noop,
        removeBlock: noop,
        acknowledgeDeviation: noop,
        revokeDeviation: noop,
    };
}
