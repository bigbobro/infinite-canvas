...
test("系统 prompt 含配色护栏：回避紫蓝渐变式 AI 默认配色，禁用纯黑纯白背景/表面色", () => {
    const messages = buildPptStyleDirectionMessages(transitStationInput());
    assert.match(messages[0].content, /紫色到蓝色/);
    assert.match(messages[0].content, /#000000/);
    assert.match(messages[0].content, /#FFFFFF/);
    assert.doesNotMatch(messages[0].content, /"surface":"#FFFFFF"/);
});

test("严格解析拒绝非三个候选、不完整 Contract，容错解析下代码围栏 JSON 不再报错，且不会自动重试", async () => {
    const input = transitStationInput();
    ...
    const fenced = parsePptStyleDirectionResponse(`\`\`\`json\n${candidateEnvelope()}\n\`\`\``, input);
...
test("JSON 容错解析：裸 JSON、代码围栏、说明文字包裹均可解析，真正非 JSON 仍报错", () => {
    const input = transitStationInput();
    const raw = candidateEnvelope();

    const plain = parsePptStyleDirectionResponse(raw, input);
    assert.equal(plain.length, 3);

    const fencedWithLang = parsePptStyleDirectionResponse(`\`\`\`json\n${raw}\n\`\`\``, input);
    assert.equal(fencedWithLang.length, 3);

    const fencedNoLang = parsePptStyleDirectionResponse(`\`\`\`\n${raw}\n\`\`\``, input);
    assert.equal(fencedNoLang.length, 3);

    const wrapped = parsePptStyleDirectionResponse(`这是为你准备的视觉方向：\n${raw}\n如有需要可以继续调整。`, input);
    assert.equal(wrapped.length, 3);

    assert.throws(() => parsePptStyleDirectionResponse("抱歉，这次没有生成有效的视觉方向建议。", input), /不是严格 JSON/);
});
