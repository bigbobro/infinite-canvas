...
// [二开] maolao 异步生图：句柄落盘 + 重载续轮询，实现全部在该模块内
import { useCanvasImageTasks, type ImageTaskHandle } from "@/pages/canvas/use-canvas-image-tasks";
...
import { CanvasPptPanel } from "@/components/canvas/canvas-ppt-panel"; // [二开] PPT 工作台
...
function InfiniteCanvasPage() {
    ...
    const pptOverlayOpen = useCanvasUiStore((state) => state.pptOverlayOpen); // [二开] #34：PPT 精修台/终审打开时隐藏节点悬浮工具条
        ...
        setNodes((prev) =>
            prev.map((node) =>
                affectedNodeIds.has(node.id) && node.metadata?.status === NODE_STATUS_LOADING
                    ? // [二开] imageTask 一并清除：用户已放弃该任务，留着只是死数据（idle 节点不会被续轮询拾起）。
                      { ...node, metadata: { ...node.metadata, status: NODE_STATUS_IDLE, errorDetails: undefined, imageTask: undefined } }
                    : node,
            ),
        );
    }, []);
    ...
    const handleGenerateNode = useCallback(
        async (nodeId: string, mode: CanvasNodeGenerationMode, prompt: string) => {
            ...
            if (sourceNode && builtinPanel?.writeBackToSelf && builtinPanel.mode === "image") {
                ...
                // [二开] imageTask 置空：这是一次全新生成，上一轮的任务句柄已失效，避免刷新捞回旧任务。
                setNodes((prev) => prev.map((node) => (node.id === nodeId ? { ...node, metadata: { ...node.metadata, prompt: scene, status: NODE_STATUS_LOADING, errorDetails: undefined, imageTask: undefined } } : node)));
                try {
                    ...
                    // [二开] 插件节点 writeBackToSelf 同样是节点归属的生成入口，任务创建后立即落盘句柄
                    const requestOptions = { signal: controller.signal, onTaskCreated: (handle: ImageTaskHandle) => persistImageTask(nodeId, handle, generationConfig.model) };
                    ...
                    if (!isGenerationCanceled(error)) {
                        const errorDetails = error instanceof Error ? error.message : "生成失败";
                        message.error(errorDetails);
                        setNodes((prev) => prev.map((node) => (node.id === nodeId ? { ...node, metadata: { ...node.metadata, status: NODE_STATUS_ERROR, errorDetails, imageTask: undefined } } : node)));
                    }
                } finally {
                        ...
                        ...prev.map((node) =>
                            node.id === nodeId
                                ? isConfigNode
                                    ? {
                                          ...node,
                                          // imageTask 置空：这是一次全新生成，上一轮的任务句柄已失效，
                                          // 留着会让「设为 loading」到「新句柄落盘」之间的刷新捞回旧图。
                                          metadata: { ...node.metadata, prompt: effectivePrompt, status: NODE_STATUS_LOADING, errorDetails: undefined, imageTask: undefined },
                                      }
                                    : isEmptyImageNode
                                      ? {
                                            ...node,
                        ...
                        targetIds.map(async (targetId) => {
                            try {
                                const requestOptions = {
                                    signal: controller.signal,
                                    onTaskCreated: (handle: ImageTaskHandle) => persistImageTask(targetId, handle, generationConfig.model),
                                };
    ...
    const handleRetryNode = useCallback(
        async (node: CanvasNodeData) => {
            ...
            setNodes((prev) => prev.map((item) => (item.id === node.id ? { ...item, metadata: { ...item.metadata, status: NODE_STATUS_LOADING, errorDetails: undefined, imageTask: undefined } } : item)));
    ...
    return (
        <main className="flex h-full min-h-0 overflow-hidden" style={{ background: theme.canvas.background, color: theme.node.text }}>
            ...
            {/* [二开] data-canvas-viewport：上游侧栏可调宽/收起后 main 已不等于画布可视区，PPT 结构定位改读这个真实 section rect */}
            <section className="relative min-w-0 flex-1 overflow-hidden" data-canvas-viewport>
                <CanvasTopBar
