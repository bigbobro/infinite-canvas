...
import { AlertTriangle } from "lucide-react"; // [二开] 可信来源警告图标
...
export function PromptSourceEditorDrawer({ open, source, onSave, onClose }: { open: boolean; source: PromptSource | null; onSave: (source: PromptSource) => void; onClose: () => void }) {
    ...
    return (
        ...
        >
            ...
            {/* [二开] 可信来源警告：脚本以 new Function 在页面上下文执行，不做沙箱隔离 */}
            <div className="mt-6 flex items-start gap-2 rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs leading-5 text-amber-700 dark:text-amber-400">
                <AlertTriangle className="mt-0.5 size-4 shrink-0" />
                <span>脚本在当前页面上下文中直接执行，可访问浏览器本地数据（包含 AI API Key）。请仅使用你信任来源的脚本。</span>
            </div>
