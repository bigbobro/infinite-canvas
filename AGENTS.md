...
- 每次开发任务完成后必须执行一套项目已有的完整本地验证：至少运行项目自动化测试、typecheck、build，并在已配置的 localhost 环境完成与改动相关的真实路径测试或冒烟；不要把验证留给用户。涉及真实模型调用时，仍须遵守“项目注意事项”的预算确认规则。
- 不要改无关文件，不要顺手重构。
...
- 本规则作为当前单人 + AI 协作阶段的默认做法；若团队协作、部署方式或上游关系改变，再由用户确认调整。当前不引入长期 `develop` / `release` 分支。
- `upstream/main` 只代表上游原项目，保持只读；`origin/main` 是本仓二开的远程正式版本；本地 `main` 只接收已完成验收、可发布的代码，不直接在 `main` 上开发功能。
- 每个开发任务从最新本地 `main` 创建一个短期分支，按 `feat/<task>`、`fix/<task>`、`hotfix/<task>` 命名；一个分支只承载一个任务，不混入无关改动。
- 开发过程中允许 WIP 提交；合入前必须整理为少量语义清晰的提交，展示 commit plan 并取得用户确认。整理后的功能分支默认以 fast-forward 方式合入本地 `main`。
- 大型或跨会话任务可以把功能分支推到 `origin` 作为备份、协作或评审入口；推送功能分支不等于发布。任何客户材料、API Key、浏览器导出和本地 Trellis 敏感记录仍不得进入 Git。
- 标准收尾顺序：完成测试和文档 → 用户确认提交计划 → 提交功能分支 → 合入本地 `main` → 用户明确确认后推送 `origin/main` → 校验本地与远程提交一致 → 再清理已合并分支。
- 功能分支合入 `main` 后，功能代码已经属于 `main`；分支名只是提交指针。确认本地和远程 `main` 都包含目标提交后，可在用户确认下删除已合并的本地/远程功能分支，不长期保留已完成分支。
- 上游更新必须单独建立 `sync/upstream-vX` 分支：先评估 release notes、侵入点和冲突，再把 `upstream/main` 合入该同步分支并执行真实路径验收；禁止把上游直接合进正在开发的功能分支。验收通过并经用户确认后，才合入本地 `main` 和推送。
- 对外汇报统一区分四种状态：①功能分支开发中，未影响正式版；②测试完成，等待合并确认；③已合入本地 `main`，尚未推送；④已推送 `origin/main`，远程正式代码已更新。
...
- 任何会消耗用户 API Key 的测试或验收，执行前必须先列出调用预算，至少分别说明文本、文生图、图生图各调用多少次及预计上限，并标明哪些步骤可用假数据或静态检查替代；只有用户明确确认预算后才能发起真实模型请求，未确认时不得试调用。
- 上游合并后不能只以 `typecheck` / `tsc` 通过作为完成依据；凡侵入点清单中标记为“改上游既有行为”的项，必须执行对应的真实运行路径验收。`resetInterruptedGeneration` 必须在异步生图处于 loading 时刷新页面，确认继续轮询原 task、不重新提交任务，并能正确回填结果。
- PPT 工作台与结构画布切换相关改动，不能只确认界面可见；返回结构画布后必须实际拖动画布、缩放、选中和点击节点，确认没有不可见弹层拦截指针事件。
